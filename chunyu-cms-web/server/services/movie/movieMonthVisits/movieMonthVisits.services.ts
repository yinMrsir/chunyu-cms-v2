  ...
  async updateVisits() {
    ...
    for (const movie of moviesWithVisits) {
      ...
      const lastWeekRecord = await db.query.movieMonthVisitsTable.findFirst({
        where: eq(movieMonthVisitsTable.monthNumber, lastYearNumber + '-' + lastWeekNumber)
      });
