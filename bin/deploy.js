...
const mainProcess = () => {
  ...
  execSync(`cd ${adminDir} && pnpm install && pnpm build:single`, {
    stdio: "inherit",
  });
  console.log("chunyu-cms-admin构建完成");

  // 将chunyu-cms-admin/dist目录下所有文件复制到目标目录
  execSync(`cp -r ${path.join(adminDir, "dist")}/* ${targetDir}`, {
    stdio: "inherit",
  });
  console.log("文件复制完成");
  ...
  // 执行pm2启动命令
  execSync(`cd ${webDir} && pm2 start pm2.config.cjs`, { stdio: "inherit" });
  console.log("pm2启动完成");
};
